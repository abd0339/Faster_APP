...
public class OrderService {
        ...
        // status endpoint.
        // ─────────────────────────────────────────────────
        private static final Map<Order.OrderStatus, Set<Order.OrderStatus>> ALLOWED_TRANSITIONS = new EnumMap<>(
        ...
        // totalPrice is now MANUAL — trusted merchant input, since
        // only the merchant sets their own order's sale price (the
        // original C2 exploit was about a CUSTOMER faking a price
        // on ANY merchant's order, which this is not).
        //
        // deliveryFee stays 100% server-computed via PricingService,
        // exactly as before — that part never changes, regardless
        // of order type.
        // ─────────────────────────────────────────────────
        @Transactional
        public Order createO2OOrder(Long merchantId,
                ...
                // No stock decrement here anymore — O2O is a manual
                // price entry, not tied to specific catalog item
                // quantities, so there's nothing to decrement.
        ...
        public Order updateStatus(Long orderId,
                ...
                Set<Order.OrderStatus> allowedNext = ALLOWED_TRANSITIONS.getOrDefault(
                                order.getStatus(),
                                EnumSet.noneOf(Order.OrderStatus.class));
        ...
        public Order disputeOrder(Long orderId,
                ...
                boolean isParty = (order.getMerchant() != null &&
                                order.getMerchant().getId().equals(requesterId))
        ...
        // ─────────────────────────────────────────────────
        // UPDATE TRACKING LOCATION (public — O2O bridge link)
        // NEW — when an offline customer opens their tracking
        // link and shares their location, this recomputes the
        // real distance-based delivery fee instead of leaving
        // it at the flat fallback that PricingService uses when
        // no coordinates are available yet at order creation.
        //
        // Only allowed before the order is DELIVERED/CANCELLED —
        // recalculating money after that point doesn't make sense
        // and could interfere with commission already recorded.
        // ─────────────────────────────────────────────────
        @Transactional
        public Order updateTrackingLocation(
                        String trackingCode, Double lat, Double lng) {
                ...
                // Recompute delivery fee, commission, and grand total
                // now that we have real coordinates on both ends
                ...
                // Let the merchant/driver know the price just updated
                messagingTemplate.convertAndSend(
                                "/topic/order/" + saved.getId(),
                                buildStatusUpdate(saved));

                System.out.println(
                                "📍 Location confirmed for " + trackingCode
